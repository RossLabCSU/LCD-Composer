...
def main(args):
    ...
    frequency_output.write('\t'.join( ['File of Origin', 'Total # of Proteins in Proteome', 'Total # of LCDs Idenitified', 'Raw Number of Proteins with LCD', 'Percentage of Proteins with LCD', 'ProtIDs with LCD (underscore-delimited)'] ) + '\n')
    ...
    for (dirname, dir, files) in os.walk('.'):
        ...
        for file in files:
            ...
            if file.endswith('.fasta') or file.endswith('.fsa') or file.endswith('.fa') or file.endswith('.fna') or file.endswith('.fnn') or file.endswith('.faa') or file.endswith('.frn'):
                ...
                frequency_output.write('\t'.join( [file, str(total_seqs), str(lcd_count), str(len(prots_with_lcd)), str( len(prots_with_lcd) / total_seqs * 100 ), '_'.join(prots_with_lcd)] ) + '\n')
                completed += 1
