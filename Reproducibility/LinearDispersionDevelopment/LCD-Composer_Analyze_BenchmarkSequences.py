...
Description:
    ...
    Refer to Cascarina et. al. (2021) NAR Genomics and Bioinformatics (https://academic.oup.com/nargab/article/3/2/lqab048/6285187) for a complete 
...
def main(args):
    ...
    all_aas = 'GX'  # USE THIS AMINO ACID ALPHABET FOR BENCHMARK SEQUENCES, WHICH ARE COMPOSED ENTIRELY OF G AND X RESIDUES
    ...
    if args.minimal_output==True:
        output = open(args.results_file + '_MinMax_Sequences_Only.tsv', 'w')
    else:
    ...
    proteome_size = sum( [ calc_combinations(seq_len, r) for r in range(1, mut_range+1) ] )
    ...
    for num_muts in range(1, mut_range+1):
        for position_comb in combinations([x for x in range(seq_len)], num_muts):
            pep_num += 1
            id = 'PepNum' + str(pep_num) + '_NumMuts' + str(num_muts)
            seq = 'G'*seq_len
            ...
            for i in range(len(seq) - win_size+1):
                ...
                #PERFORM FULL CALCULATIONS
                    ...
                    #QUICK CALCULATION FOR WINDOWS THAT ARE 100% AAs OF INTEREST - COMMENTED OUT FOR ANALYZING BENCHMARK SEQUENCES BECAUSE SEQUENCES WITH 100% COMPOSITION OF A SINGLE CHARACTER ARE NEVER GENERATED IN THIS SCRIPT.
            ...
            trimmed_seqs, trimmed_boundaries = seqs[:], domain_boundaries[:] # MODIFIED TO OMIT TRIMMING STEP AND SIMPLY SAVE COPIES OF THE seqs AND domain_boundaries LISTS.
            ...
            if args.minimal_output==True and final_stddevs[0] > 0.000001 and final_stddevs[0] < 0.999999:
                continue
...
def get_args(arguments):
    ...
    parser.add_argument('-r', '--mutation_range', type=int, default=19, 
                        help="""Mutation range (integer). Represents the maximum number of "mutations" that you wish to generate sequences for.
                                    For example, benchmark sequences that are 30aa sequences start with 29 G residues and 1 X residue (representing 1 "mutation").
                                    ...
                                    It is recommended that you only set the mutation_range to a maximum of 1/2 the sequence_length, 
                                    since the linear dispersion for all subsequent sequences will be identical to the reciprocal 
                                    sequence which will already be analyzed with the mutations 1-15.
    ...
    parser.add_argument('-l', '--sequence_length', type=int, default=20, 
                        help="""Sequence length (integer). Represents the fixed length of benchmark sequences that you wish to analyze.""")
                        
    parser.add_argument('-m', '--minimal_output', action='store_true',
                        help="""Only outputs sequences that have a linear dispersion equal to 0 or 1 (representing minimum and maximum linear dispersion, respectively).
                                """)
