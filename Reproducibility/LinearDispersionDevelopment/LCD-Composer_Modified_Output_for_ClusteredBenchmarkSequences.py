...
def main(args):
    ...
    if args.verbose:
        output.write('\t'.join( ['Protein ID','Full Sequence', '\t'.join([aa_group + '_Composition' for aa_group in amino_acids]), 'Combined Linear Dispersion'] ) + '\n')
    else:
        output.write('\t'.join( ['Protein ID','Full Sequence','Domain Sequences','Domain Boundaries'] ) + '\n')
    ...
    for id, seq in fasta_parser(h):
        ...
        if len(trimmed_boundaries) > 0 and args.verbose==False:
            ...
            output.write('\t'.join( [id, seq, '_'.join(trimmed_seqs), '_'.join(trimmed_boundaries)]) + '\n')
        # elif len(trimmed_boundaries) > 0 and args.verbose==True:
            # output.write('\t'.join( [id, seq, '\t'.join( ['_'.join([str(round(x, 2)) for x in group_comps[aa_group]]) for aa_group in amino_acids] ), '_'.join([str(round(x, 4)) for x in normed_stddevs]) ] ) + '\n')
        else:
            continue
