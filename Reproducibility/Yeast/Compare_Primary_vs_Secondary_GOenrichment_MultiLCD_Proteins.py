...
def main():
    ...
    output = open('Primary_vs_Secondary_GOenrichment_MultiLCD_Proteins.tsv', 'w')
    output.write('\t'.join(['GO term', 'Primary LCD Class', 'Secondary LCD Class', 'Effect of Subclassification']) + '\n')
    ...
    for aa in amino_acids:
        ...
        for go_term in list(p_gos['GO term']):
            ...
            if effect == 'Retained':
                output.write('\t'.join([go_term, aa, '_'.join(maintained_secondary_aas), effect]) + '\n')
            else:
                output.write('\t'.join([go_term, aa, 'None', effect]) + '\n')
        ...
        for sec_aa in amino_acids.replace(aa, ''):
            ...
            for go_term in list(final_s_gos['GO term']):
                if go_term not in list(p_gos['GO term']):
                    effect = 'New'
                    output.write('\t'.join([go_term, aa, sec_aa, effect]) + '\n')
...
def get_primary_GOs():
    ...
    pf = open('PrimaryLCD_GOenrichment_data.dat', 'wb')
    pickle.dump(df, pf)
    pf.close()
...
def get_secondary_GOs():
    ...
    pf = open('SecondarySubclassLCD_GOenrichment_data.dat', 'wb')
    pickle.dump(df, pf)
    pf.close()
