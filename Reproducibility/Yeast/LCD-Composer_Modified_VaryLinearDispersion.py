...
def main(args):
    ...
    time_output = open('ComputationTimes_DispThresholdVaried.txt', 'w')
    
    all_aas = 'ACDEFGHIKLMNPQRSTVWY'
    win_size, amino_acids, other_aas, comp_thresholds, disp_threshold, ign_disp = get_params(args)
    comp_threshold = comp_thresholds[0]
    
    for aa in all_aas:
        amino_acids = [aa]
        combined_aas = aa
        other_aas = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ*_'.replace(aa, '')
        for disp_threshold in [x/10 for x in [0, 1, 2, 3, 4, 6, 7, 8, 9, 10]]:       # disp_threshold == 0.5 IS CALCULATED BY THE LCD-Composer_Modified_VaryWindowSize.py SCRIPT

            disp_threshold = disp_threshold - 0.000001      # ADJUST FOR FLOATING POINT COMPARISONS
            
            results_filename = 'Scerevisiae_' + aa + '_' + str(int(round(comp_threshold, 0))) + '-CompositionThreshold_' + str(win_size) + '-WindowSize_' + str(round(disp_threshold, 1)).replace('.', 'p').replace('-', '') + '-LinearDispersionThreshold_LCD-Composer_RESULTS'
