...
def main():
    ...
    for aa in amino_acids:
        for secondary_aa in amino_acids.replace(aa, ''):
            ...
            lcd_prots = []
            for line in h:
                id, *remainder = line.rstrip().split('\t')
                junk, uniprot_id, junk = id.split('|')
                if uniprot_id not in high_homol_prots[aa][secondary_aa]:
                    lcd_prots.append(uniprot_id)
            ...
            if len(lcd_prots) > 0:
                ...
                file_name = organism + '_' + aa + '-primary_' + secondary_aa + '-secondary_GO_RESULTS_80percHomologyProtsRemoved.tsv'
