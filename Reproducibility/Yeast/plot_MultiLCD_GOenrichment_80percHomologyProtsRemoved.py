...
def main():
    ...
    h = open('MultiLCD_Proteins_GOenrichment_80percHomologyProtsRemoved.tsv')
...
def stacked_barchart(df):
    ...
    plt.xlim(-0.6, 11.6)
    plt.ylim([0, 1.25])
    plt.xticks([x for x in range(len(sorted_aas[:-9])+1)], labels=list(sorted_aas)[:-9] + ['All LCDs'], fontname='Arial', fontsize=16)
    ...
    plt.savefig('Fig S15C - MultiLCD_Proteins_GOenrichment_Comparison_80perHomologyProtsRemoved.tiff', bbox_inches ='tight', dpi=600)
    plt.close()
