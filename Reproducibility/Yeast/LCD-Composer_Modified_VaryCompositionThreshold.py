...
def main(args):
    ...
    time_output = open('ComputationTimes_CompThresholdVaried.txt', 'w')
    ...
    for aa in all_aas:
        ...
        for comp_threshold in [10, 20, 30, 50, 60, 70, 80, 90, 100]:    # comp_threshold == 40 IS CALCULATED BY THE LCD-Composer_Modified_VaryWindowSize.py SCRIPT
            
            comp_thresholds = [comp_threshold - 0.000001]      # ADJUST FOR FLOATING POINT COMPARISONS
            ign_disp = comp_threshold + ((100 - comp_threshold)/2) - 0.000001
            
            results_filename = 'Scerevisiae_' + aa + '_' + str(int(round(comp_threshold, 0))) + '-CompositionThreshold_' + str(win_size) + '-WindowSize_' + str(round(disp_threshold, 1)).replace('.', 'p') + '-LinearDispersionThreshold_LCD-Composer_RESULTS'
            
            start_time = str(datetime.datetime.now())
            time_output.write(results_filename + '\n')
            time_output.write('Start time: ' + start_time + '\n')

            print('Start time:', start_time, amino_acids, disp_threshold)
            ...
            output = open(results_filename + '.tsv', 'w')
            output_params_header(output, win_size, amino_acids, comp_thresholds, disp_threshold, ign_disp)
            ...
            time_output.write('End time: ' + end_time + '\n\n')
...
def output_params_header(output, win_size, amino_acids, comp_thresholds, disp_threshold, ign_disp):
...
def get_args(arguments):
    ...
    parser.add_argument('-a', '--amino_acids', type=str, default='A',
