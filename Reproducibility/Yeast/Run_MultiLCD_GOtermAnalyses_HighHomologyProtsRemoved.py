...
def main():
    ...
    for aa in amino_acids:
        for secondary_aa in amino_acids.replace(aa, ''):
            ...
            lcd_prots = get_lcd_prots(aa, secondary_aa)
            ...
            if len(lcd_prots) > 0:
                ...
                file_name = organism + '_' + aa + '_' + secondary_aa + '_MultiLCD_GO_RESULTS_80percHomologyProtsRemoved.tsv'
                g.wr_tsv(file_name, g_res)

def get_lcd_prots(aa, secondary_aa):

    prots = []
    try:
        h = open('Scerevisiae_' + aa + '_' + secondary_aa + '_MultiLCD_proteins_80percHomologyProtsRemoved.txt')
    except:
        return prots
    for line in h:
