...
def main():
    ...
    output = open('Primary_vs_Secondary_GOenrichment_80percHomologyProtsRemoved.tsv', 'w')
...
def get_primary_GOs():
    ...
    for aa in amino_acids:
        for secondary_aa in amino_acids.replace(aa, ''):
            try:
                h = open('Scerevisiae_' + aa + '-primary_' + secondary_aa + '-secondary_GO_RESULTS.tsv')
            except:
                continue
            header = h.readline()
            for line in h:
...
def get_secondary_GOs():
    ...
    for aa in amino_acids:
        for secondary_aa in amino_acids.replace(aa, ''):
            try:
                h = open('Scerevisiae_' + aa + '-primary_' + secondary_aa + '-secondary_GO_RESULTS_80percHomologyProtsRemoved.tsv')
            except:
                continue
            header = h.readline()
            for line in h:
