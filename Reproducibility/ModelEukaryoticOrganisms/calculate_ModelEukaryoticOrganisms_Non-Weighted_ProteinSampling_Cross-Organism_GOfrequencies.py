...
def main():
    ...
    output = open('Table S6 - Combined_GOtermResults_ModelEukaryoticOrganisms_Non-Weighted_ProteinSampling.tsv', 'w')
    ...
    freqs_out = open('Table S7 - ModelEukaryoticOrganisms_Cross-Organism_GOfrequencies_Non-Weighted_ProteinSampling.csv', 'w')
...
def get_RandomGO_Results(organism, df, aa, output):
    ...
    h = open(organism + '_RandomlySampledProteins_All_Iterations_GO_RESULTS.tsv')  
    header = h.readline()
