...
def plotting(df):
    ...
    for aa in amino_acids.replace('W', ''):
        new_df = {'LCD Class':[],
                'Number of Organisms':[],
                'Count':[],
                'Category':[]
        ...
        plt.xlim(-1, 6)
        plt.xticks([x for x in range(7)], labels=[str(x) for x in range(1, 7)], fontname='Arial', fontsize=14)
        
        if aa == 'F':
            plt.yticks([0, 2, 4, 6, 8], labels=['0', '2', '4', '6', '8'], fontname='Arial', fontsize=14)
        elif aa in 'IN':
            plt.yticks([0, 5, 10, 15, 20, 25], labels=['0', '5', '10', '15', '20', '25'], fontname='Arial', fontsize=14)
        elif aa in 'KL':
            plt.yticks([0, 10, 20, 30, 40, 50], labels=['0', '10', '20', '30', '40', '50'], fontname='Arial', fontsize=14)
        elif aa == 'M':
            plt.yticks([0, 1, 2], labels=['0', '1', '2'], fontname='Arial', fontsize=14)
        elif aa == 'S':
            plt.yticks([0, 25, 50, 75, 100, 125], labels=['0', '25', '50', '75', '100', '125'], fontname='Arial', fontsize=14)
        elif aa == 'V':
            plt.yticks([0, 1, 2, 3, 4, 5, 6], labels=['0', '1', '2', '3', '4', '5', '6'], fontname='Arial', fontsize=14)
        elif aa == 'Y':
    ...
    plt.savefig('Fig S10 - ModelEukaryoticOrganisms_EXCLUDED-ANNOTS_GOresults.tiff', bbox_inches ='tight', dpi=600)
    plt.close()  
