...
def main():
    ...
    for i in range(len(abbrevs)):
        prots_output = open(abbrevs[i] + '_RandomlySelectedProts_All_Iterations_ALL-ISOFORMS.csv', 'w')
        prots_output.write(','.join( ['LCD Class', 'Iteration Number', 'Sampled Proteins (underscore-delimited)'] ) + '\n')
        output = open(abbrevs[i] + '_RandomlySampledProteins_All_Iterations_GO_RESULTS_ALL-ISOFORMS.tsv', 'w')
