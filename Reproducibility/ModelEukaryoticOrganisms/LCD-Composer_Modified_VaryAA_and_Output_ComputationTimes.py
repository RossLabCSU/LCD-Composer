...
def main(args):
    ...
    time_output = open('ComputationTimes_ModelOrganisms_IsoformsIncluded.txt', 'w')
    ...
    abbrevs = ['Ecoli', 'Scerevisiae', 'Celegans', 'Dmelanogaster', 'Drerio', 'Xlaevis', 'Mmusculus', 'Hsapiens']
    ...
    for j in range(len(abbrevs)):
        proteome_file = proteomes[j] + '.fasta'
        organism = abbrevs[j]
        for aa in all_aas:
            amino_acids = [aa]
            combined_aas = aa
            ...
            results_filename = organism + '_' + aa + '_LCD-Composer_RESULTS_timed_IsoformsIncluded'
            
            start_time = str(datetime.datetime.now())
            time_output.write(results_filename + '\n')
            time_output.write('Start time: ' + start_time + '\n')

            print('Start time:', start_time, organism, amino_acids, disp_threshold)
            ...
            proteome_size = get_proteome_size(proteome_file)
