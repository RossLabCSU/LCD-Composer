...
def plotting(df):
    ...
    # rand_samp_counts = pickle.load(open('RandomSampling_100k_Iterations_CrossSpecies_GOterm_Estimation.dat', 'rb'))

    stats_output = open('TableS9_Statistics_for_CrossSpecies_NumGOterms_vs_RandomSampling.csv', 'w')
    ...
    plt.savefig('Fig S11 - CrossSpecies GOterm Sampling Estimation.tiff', bbox_inches ='tight', dpi=600)
    plt.close()  
...
def random_go_sampling():
    ...
    cross_spec_counts = {}
    for aa in amino_acids:
        print('Running', aa, 'Random Sampling 100k iterations', str(datetime.datetime.now()))
    ...
    pf = open('RandomSampling_100k_Iterations_CrossSpecies_GOterm_Estimation.dat', 'wb')
