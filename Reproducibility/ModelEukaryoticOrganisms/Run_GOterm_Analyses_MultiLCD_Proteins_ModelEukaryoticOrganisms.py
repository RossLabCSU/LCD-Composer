        ...
        for aa in amino_acids:
            for secondary_aa in amino_acids.replace(aa, ''):
                ...
                if len(lcd_prots) > 0:
                    ...
                    file_name = organism + '_' + aa + '_' + secondary_aa + '_MultiLCD_Proteins_GO_RESULTS.tsv'
                    g.wr_tsv(file_name, g_res)
           
    
def get_hits(organism, aa, secondary_aa):

    prots = []
    try:
        h = open(organism + '_' + aa + '_' + secondary_aa + '_MultiLCD_Proteins.txt')
    except:
        return prots
    for line in h:
