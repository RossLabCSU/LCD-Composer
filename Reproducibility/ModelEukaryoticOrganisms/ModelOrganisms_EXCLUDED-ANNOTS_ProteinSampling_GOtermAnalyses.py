...
def main():
    ...
    for i in range(len(abbrevs)):
        prots_output = open(abbrevs[i] + '_RandomlySelectedProts_All_Iterations_EXCLUDED-ANNOTS.csv', 'w')
        prots_output.write(','.join( ['LCD Class', 'Iteration Number', 'Sampled Proteins (underscore-delimited)'] ) + '\n')
        output = open(abbrevs[i] + '_RandomlySampledProteins_All_Iterations_GO_RESULTS_EXCLUDED-ANNOTS.tsv', 'w')
        ...
        for iteration in range(1, 1001):
            ...
            for aa in amino_acids:
                ...
                random_prots = prot_sample(full_proteome_lists[ abbrevs[i] ], num_prots)
...
def prot_sample(prot_list, num_prots):
    
    sampled_prots = []
    while len(sampled_prots) < num_prots:
        prot_ind = np.random.choice(len(prot_list))
        prot = prot_list[prot_ind]
...
def get_assoc(abbrev):
    ...
    h = open(abbrev + '_ISS_ISO_ISA_ISM_IEA_Excluded.gaf')
