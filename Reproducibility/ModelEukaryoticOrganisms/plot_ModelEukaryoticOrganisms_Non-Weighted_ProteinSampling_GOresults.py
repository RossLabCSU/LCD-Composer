...
def output_table(df):
    ...
    output = open('Table S4 - ModelEukaryoticOrganisms_CrossSpecies_Enriched_GOterms_Comparisons.tsv', 'w')
    output.write('LCD Class\tGO Term\tGO id\tGO Category\tOrganisms with GO Term Enchriched\tNumber of Organisms with GO Term Enriched\n')
    
    for aa in amino_acids:
        temp_df = df[df['Amino Acid'] == aa]
        for go_term in set(list(temp_df['GO term'])):
            organisms = []
            go_id = ''
            co_cat = ''
            for i in range(len(temp_df['GO term'])):
                if list(temp_df['GO term'])[i] == go_term:
                    organisms.append(list(temp_df['Proteome'])[i])
                    go_id = list(temp_df['GO ID'])[i]
                    go_cat = list(temp_df['Category'])[i]

            output.write('\t'.join([aa, go_term, go_id, go_cat, ', '.join(organisms), str(len(organisms))]) + '\n')
...
def plotting(df):
    ...
    plt.savefig('Fig 6 - ModelEukaryoticOrganisms_Non-Weighted_ProteinSampling_GOresults.tiff', bbox_inches ='tight', dpi=600)
    plt.close()  
...
def get_sampling_data():
    ...
    h = open('Table S7 - ModelEukaryoticOrganisms_Cross-Organism_GOfrequencies_Non-Weighted_ProteinSampling.csv')
    header = h.readline()
    for line in h:
