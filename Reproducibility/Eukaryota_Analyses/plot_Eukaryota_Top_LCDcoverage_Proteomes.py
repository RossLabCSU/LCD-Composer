...
def main():
    ...
    for line in h:
        ...
        # SKIP WOLVERINE PROTEOME...THIS APPEARS TO BE AN INCORRECT EUKARYOTIC PROTEOME CONTAINING ONLY 23 PROTEINS, AND THIS HEAVILY SKEWS THE LCD DATA
        if proteome == 'UP000269945_48420':
            continue
    ...
    write_file(total_lcd_contents, lcd_contents_bytype, proteomes, life_domain)

    
def write_file(total_lcd_contents, lcd_contents_bytype, proteomes, life_domain):

    output = open('PartOfTable S1_' + life_domain + ' Percentage_LCDcoverage_wideform.tsv', 'w')
...
def plotting(total_lcd_contents, lcd_contents_bytype, proteomes, life_domain):
    ...
    plt.savefig('Fig 5C - ' + life_domain + '_Top' + str(num_proteomes) + '_LCDproteins.tiff', bbox_inches='tight', dpi=600)
    plt.close()
