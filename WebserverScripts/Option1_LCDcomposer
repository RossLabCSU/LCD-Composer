...
def LCDcomposer_NewDispMethod(random_id, fasta_files, win_size, amino_acids, other_aas, comp_thresholds, disp_thresholds, disp_method, ign_disp):
    ...
    for file_index, file in enumerate(fasta_files):
        ...
        for id, seq in fasta_parser(h):
            ...
            if len(trimmed_boundaries) > 0:
                if file_index == 0:     # ONLY ADD PROTS FOR THE MAIN REFERENCE PROTEOME (NOT THE _additional FILE).
...
def LCDcomposer_AutoDetect(random_id, fasta_files, win_size, amino_acids, other_aas, comp_thresholds, disp_threshold, disp_method, ign_disp):
    ...
    lcd_prots = {aa:[] for aa in all_aas}
...
def get_params( args ):
    ...
    return fasta_files, win_size, amino_acids, other_aas, comp_thresholds, disp_threshold, disp_method, ign_disp, run_autodetect
...
def main(args):
    ...
    fasta_files, win_size, amino_acids, other_aas, comp_thresholds, disp_threshold, disp_method, ign_disp, run_autodetect = get_params(args)
    
    random_id = random.randint(0, 10000000)

    if run_autodetect:
        LCDcomposer_AutoDetect(random_id, fasta_files, win_size, amino_acids, other_aas, comp_thresholds, disp_threshold, disp_method, ign_disp)

    else:
        if disp_method == 'Original':
            LCDcomposer(random_id, fasta_files, win_size, amino_acids, other_aas, comp_thresholds, disp_threshold, disp_method, ign_disp)
        else:
            LCDcomposer_NewDispMethod(random_id, fasta_files, win_size, amino_acids, other_aas, comp_thresholds, disp_threshold, disp_method, ign_disp)
...
if __name__ == '__main__':
    import sys, argparse, datetime, math, statistics, random
