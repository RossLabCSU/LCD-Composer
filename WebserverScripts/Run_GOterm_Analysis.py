...
def main(args):
    ...
    lcd_resultsfile, fasta_file, gaf_file = get_params(args)
    
    lcd_prots = get_LCDprots(lcd_resultsfile)
    
    if len(lcd_prots) == 0:
        print('\nError:\nNo LCD-containing proteins were detected in your results file. Please ensure that your LCD search results file contains at least on protein with an LCD and that the format/content of the results file remains unchanged.\n')
        exit()
    ...
    output_GOresults(lcd_resultsfile, fasta_str, g_res)

    
def get_LCDprots(filename):
...
def get_params(args):
    ...
    return lcd_resultsfile, fasta_file, gaf_file
...
def output_GOresults(filename, fasta_str, g_res):

    output = open(filename.replace('.tsv', '')+'_GOterm_Results.tsv', 'w')
...
def get_args(arguments):
    ...
    parser.add_argument('filename', help="""The name fo the file containing results from an LCD search. GO-term analysis will be run on the proteins listed in the results file.""")
