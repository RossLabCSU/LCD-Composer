...
def extract_features(query_seq, num_features):
    ...
    residues = amino_acids[:len(thresholds)]
...
def main(args):
    ...
    output.write('\t'.join( ['LCD Similarity Rank','LCD Similarity Score','Protein Description','UniProt ID (when applicable)','Domain Sequence','Domain Boundaries','Final Domain Composition','Final Domain Linear Dispersion','Highest LCD Similarity Score (i.e. best window)','Highest-scoring Segment (i.e. best window sequence)','\t'.join([aa for aa in all_aas]) ]) + '\n')
        ...
        for id, seq in fasta_parser(h):
            ...
            if len(trimmed_boundaries) > 0:
                for i in range(len(trimmed_seqs)):
                    # output_lines.append('\t'.join( [id, trimmed_seqs[i], trimmed_boundaries[i], str(round(final_comps[i], 2)), final_stddevs[i], '\t'.join([str(round(trimmed_seqs[i].count(aa) / len(trimmed_seqs[i]) * 100, 2)) for aa in all_aas]) ]))
                    # output_lines.append( [id, trimmed_seqs[i], trimmed_boundaries[i], str(round(final_comps[i], 2)), final_stddevs[i], '\t'.join([str(round(trimmed_seqs[i].count(aa) / len(trimmed_seqs[i]) * 100, 2)) for aa in all_aas]) ])
...
def rank_LCDs(query_seq, hit_lcd_seqs, win_size, output_lines, output):
    ...
    max_core_scores = []
    max_core_seqs = []
    max_dissimilarity = determine_max_dissimilarity(query_seq)
    for lcd_seq in hit_lcd_seqs:
        ...
        for i in range(range_val):
            ...
            core_score = calc_similarity_EuclideanDistance(query_seq, window)
            if core_score > max_core_score:
                max_core_score = core_score
                max_core_seq = window[:]
        max_core_scores.append(max_core_score / max_dissimilarity * 100)
        max_core_seqs.append(max_core_seq)
        similarity_score = calc_similarity_EuclideanDistance(query_seq, lcd_seq)
        scores.append(similarity_score / max_dissimilarity * 100)
    ...
    if len(scores) > 0:
        scores, max_core_scores, max_core_seqs, lines = zip(*sorted(zip(scores, max_core_scores, max_core_seqs, output_lines)))
        
        for i in range(len(lines)):
            output.write('\t'.join([str(i+1), str(scores[i])] + lines[i][:5] + [str(max_core_scores[i]), max_core_seqs[i]] + lines[i][5:]) + '\n')

    return output

    
def calc_similarity_EuclideanDistance(string1, string2):
    """Function that calculates the total composition similarity score. This score is calculated by:
...
def determine_max_dissimilarity(query_seq):
    ...
    A simple way to calculate maximum dissimilarity, then, is simply the 
    (sum of the % composition of each AA found in the query sequence)  +  100
    
    1) The first term models (%comp_query_seq - %comp_subject_seq).
    2) The "+100" accounts for 100% X in subject seq, where X is an amino acid not found in the query sequence.
    
    In rare cases where all 20 amino acids are found in the query seq, the "+100" is altered such that it now 
    represents 100 - %comp of the least-common amino acid in the query seq. This models the situation where the subject 
    sequence is a homopolymer of the least-common amino acid found in query seq.
    ...
    max_dissimilarity = calc_similarity_EuclideanDistance(query_seq, dissimilar_seq)
