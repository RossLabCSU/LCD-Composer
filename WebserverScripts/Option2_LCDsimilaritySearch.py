...
def rank_LCDs(query_seq, hit_lcd_seqs, win_size, output_lines, output):
    """Ranks discovered LCDs based on similarity to the query LCD sequence. Writes the results to the output file.
    ...
    for lcd_seq in hit_lcd_seqs:
        ...
        for i in range(range_val):
            ...
            core_score = calc_similarity_EuclDist(query_seq, window)
        ...
        similarity_score = calc_similarity_EuclDist(query_seq, lcd_seq)
...
def calc_similarity_EuclDist(string1, string2):
    """Function that calculates the total composition similarity score. This score is determined by:
...
def determine_max_dissimilarity(query_seq):
    ...
    max_dissimilarity = calc_similarity_EuclDist(query_seq, dissimilar_seq)
