...
def get_params(args):
    """Gather and define user parameters. Also generates an error message if an invalid parameter values are passed in by the user.
    ...
    if args.plot:
        ...
        if img_width < 50 or img_width > 300:
            print('\nError:\nThe image width must be a number between 50-300 (inclusive).\n')
            exit()
            
        img_height = args.img_height
        if img_height < 50 or img_height > 300:
            print('\nError:\nThe image height must be a number between 50-300 (inclusive).\n')
            exit()
        ...
        if color_palette:
            ...
            if False in matches:
                error_message += '<br />Error: Invalid color palette. The color palette must either be left blank (to use the default color palette from the Seaborn library) or be a series of valid color hex codes, each separated by an underscore ("_"), and must be of the same length as the number of specified amino acid groups.\n'
        else:
...
def get_args(arguments):
    ...
    parser.add_argument('-ir', '--img_resolution', type=float, default=600,
    ...
    parser.add_argument('-iw', '--img_width', type=float, default=150,
                        help="""Image width in mm. Default=150mm (~5.9 inches).
                        
                        Acceptable values are between 50-300.""")
                        
    parser.add_argument('-ih', '--img_height', type=float, default=150,
    ...
    parser.add_argument('-pi', '--prot_id', type=str, default='ProtID_Not_Provided',
                        help="""Protein ID. This is simply a label to use in the data (no other function).""")
